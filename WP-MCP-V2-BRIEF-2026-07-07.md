...
- **Structured outputs — tools are consumed by agents composing cascades, not just chat.** (Ross 2026-07-07: e.g. read a workback plan via Threshold MCP → build the tracker via an Excel MCP → file the outcome back via `ingest_doc`.) Every read tool returns machine-actionable JSON — dates, owners, lifecycle state, dependency edges, receipt refs — with prose as a `summary` field, never prose-only. The cascade composes client-side; Threshold needs no knowledge of other MCP servers, and the consequential-action risk stays in the OTHER servers' scopes (Threshold's token is read + append-only capture).
